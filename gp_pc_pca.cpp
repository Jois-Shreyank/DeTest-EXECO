...
std::vector<Point> gaussianProcess(const Cell& cell, int pred_dir) {
    ...
    for (int i = 0; i < num_test_side; ++i) {
        for (int j = 0; j < num_test_side; ++j) {
            ...
            coords[loc_dir1] = min_coords[loc_dir1] + u * (max_coords[loc_dir1] - min_coords[loc_dir1]);
            coords[loc_dir2] = min_coords[loc_dir2] + v * (max_coords[loc_dir2] - min_coords[loc_dir2]);
...
int main(int argc, char* argv[]) {
    ...
    std::vector<Face> faces; // Original faces are read but not used in the output
    ...
    // Apply GP to each cell and collect smoothed points
    std::vector<Point> smoothed_vertices;
    std::vector<Face> new_faces; // This will remain empty
    for (auto const& [key, cell] : grid) {
        if (cell.points.size() >= min_points_per_cell) {
            // Perform PCA to decide GP direction(s)
            ...
            double eig3_ratio_eig2 = eig.eigenvalues()(2) / eig.eigenvalues()(1);

            std::vector<int> gp_directions;
            if (eig3_ratio_eig2 > 10.0) { // Points are clustered, not a clear plane
                 gp_directions.push_back(0); // X
                 gp_directions.push_back(1); // Y
                 gp_directions.push_back(2); // Z
            } else { // Points form a plane-like structure
                int pred_dir = 0;
                eig.eigenvectors().col(0).cwiseAbs().maxCoeff(&pred_dir);
                gp_directions.push_back(pred_dir);
            }

            for (int dir : gp_directions) {
                std::vector<Point> cell_smoothed_points = gaussianProcess(cell, dir);
                smoothed_vertices.insert(smoothed_vertices.end(), cell_smoothed_points.begin(), cell_smoothed_points.end());

                // --- MODIFICATION START ---
                // REMOVED: The block of code that generated new faces for the smoothed grid.
                // The `new_faces` vector will remain empty.
                // --- MODIFICATION END ---
            }
            
            // --- MODIFICATION START ---
            // REMOVED (Recommended): The original code also added the original points back in.
            // This is likely not desired for a clean point cloud output.
            // Original line was: smoothed_vertices.insert(smoothed_vertices.end(), cell.points.begin(), cell.points.end());
            // --- MODIFICATION END ---

        } else {
            // If not enough points, just keep the original ones
            smoothed_vertices.insert(smoothed_vertices.end(), cell.points.begin(), cell.points.end());
        }
    }
    std::cout << "Generated " << smoothed_vertices.size() << " smoothed vertices." << std::endl;

    // Write the smoothed vertices and new faces (which is empty) to a PLY file
    writePly(output_filename, smoothed_vertices, new_faces);
    std::cout << "Smoothed point cloud saved to " << output_filename << std::endl;
